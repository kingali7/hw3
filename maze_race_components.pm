...
sub rushy{
  ...
  my $newR = $lastR - $r;
  my $newC = $lastC - $c;
  local @rshift = ($newR, 0, 0, 0);
  local @cshift = ($newC, 0, 0, 0);    
  $self->move(0, $maze);
  $maze->explore($pos);
...
sub throughBlocked{
  ...
  if($target_c < $maze->getWidth() & $target_r < $maze->getHeight() & $target_c >= 0 & $target_r >= 0){
    if($maze->getCellContent($target_pos) eq '*' & $maze->getCellContent($tmp_p) ne '*'){
      my $newR = $target_r - $r;
      my $newC = $target_c - $c;
      local @rshift = ($newR, 0, 0, 0);
      local @cshift = ($newC, 0, 0, 0);  
    ...
    elsif($maze->getCellContent($target_pos) eq '*' & $maze->getCellContent($tmp_p) eq '*'){
      $self->move($pointTo, $maze);
    }
    else{
      $maze->explore($tmp_p);
